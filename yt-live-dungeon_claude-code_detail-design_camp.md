...
同一ランで戦死済みのyoutube_idは再参加不可。現在参加中なら重複loginとして拒否する。

以前 `@logout` した生存冒険者の再loginを許可する場合は、既存冒険者・既存所持品を再利用し、新しい精霊・加護・プールアイテムを再抽選しない。これは重複初期付与を防ぐ実装上の不変条件とする。
...
参加人数は、そのフロア開始時点の生存中かつ参加中の冒険者数とする。

最大MP、MP回復速度、Spell基本威力、MP消費、Weak、Break、Chainにはフロア・人数補正を掛けない。
