...
class CampConfigurationError(DomainError):
    """Raised when master data can't support starting a CAMP (e.g. an
    egregore's active item pool has fewer than 2 entries)."""


class LoginConfigurationError(DomainError):
    """Raised when master data can't support onboarding a new adventurer
    (no active egregores, or the drawn egregore's active item pool is
    empty)."""
