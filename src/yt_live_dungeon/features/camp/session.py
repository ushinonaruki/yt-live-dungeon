...
async def resolve_camp_session(
    session: AsyncSession, run_id: uuid.UUID, viewer_id: str, *, lock_run: bool = False
) -> ResolvedCampSession | CommandOutcome:
    """Common preconditions shared by every CAMP command:

    run.state == CAMP, an open RunCamp exists for the current floor, and
    the viewer maps to a currently alive & participating RunAdventurer.
