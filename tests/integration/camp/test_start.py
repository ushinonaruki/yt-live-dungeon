...
async def _setup_egregore_with_pool(
    spell_factory, item_factory, egregore_factory, pool_entry_factory, pool_size: int = 2
):
...
async def test_starts_camp_from_battle_run(
    ...
    egregore, _pool_ids = await _setup_egregore_with_pool(
        spell_factory, item_factory, egregore_factory, pool_entry_factory
    )
    run = await run_factory(state=RunState.BATTLE, current_floor=3)

    camp = await start_camp(
        db_session, run.id, egregore.id, now=NOW, random_source=random.Random(1)
    )
...
async def test_candidates_are_two_distinct_items(
    ...
    egregore, pool_ids = await _setup_egregore_with_pool(
        spell_factory, item_factory, egregore_factory, pool_entry_factory, pool_size=4
    )
    run = await run_factory()

    camp = await start_camp(
        db_session, run.id, egregore.id, now=NOW, random_source=random.Random(7)
    )
...
async def test_candidates_are_reproducible_with_fixed_random_seed(
    ...
    egregore, pool_ids = await _setup_egregore_with_pool(
        spell_factory, item_factory, egregore_factory, pool_entry_factory, pool_size=5
    )
    run_a = await run_factory()
    run_b = await run_factory()

    camp_a = await start_camp(
        db_session, run_a.id, egregore.id, now=NOW, random_source=random.Random(42)
    )
    camp_b = await start_camp(
        db_session, run_b.id, egregore.id, now=NOW, random_source=random.Random(42)
    )
...
async def test_pool_excludes_items_from_other_egregores(
    ...
    egregore_a = await egregore_factory(blessing_item_id=blessing_a.id)
    blessing_b = await item_factory(granted_spell_id=spell.id)
    egregore_b = await egregore_factory(blessing_item_id=blessing_b.id)

    item_for_a = await item_factory(granted_spell_id=spell.id)
    item_for_b = await item_factory(granted_spell_id=spell.id)
    await pool_entry_factory(egregore_id=egregore_a.id, item_id=item_for_a.id)
    await pool_entry_factory(egregore_id=egregore_b.id, item_id=item_for_b.id)

    pool_ids = await list_active_pool_item_ids(db_session, egregore_a.id)
...
async def test_pool_excludes_inactive_items(
    ...
    await pool_entry_factory(egregore_id=egregore.id, item_id=active_item.id)
    await pool_entry_factory(egregore_id=egregore.id, item_id=inactive_item.id)

    pool_ids = await list_active_pool_item_ids(db_session, egregore.id)
...
async def test_pool_item_ids_are_returned_in_deterministic_ascending_order(
    ...
    for item in reversed(items):
        await pool_entry_factory(egregore_id=egregore.id, item_id=item.id)
...
async def test_rerunning_for_same_run_and_floor_is_idempotent(
    ...
    second = await start_camp(
        db_session, run.id, egregore.id, now=NOW, random_source=random.Random(999)
    )
...
async def test_raises_when_pool_has_fewer_than_two_items(
    ...
    with pytest.raises(CampConfigurationError):
        await start_camp(db_session, run.id, egregore.id, now=NOW, random_source=random.Random(1))
...
async def test_camp_started_event_is_recorded(
    ...
    assert event.body["egregore_id"] == egregore.id
...
async def test_concurrent_start_camp_calls_for_same_run_and_floor_produce_one_camp():
    ...
    async with async_session_factory() as setup_session:
        ...
        egregore = Egregore(
            egregore_key=f"egregore-{uuid.uuid4().hex[:8]}",
            display_name="s",
            representative_attribute="RR",
            blessing_item_id=blessing_item.id,
        )
        setup_session.add(egregore)
        await setup_session.flush()
        ...
        for _ in range(2):
            ...
            await setup_session.flush()
            setup_session.add(EgregoreItemPoolEntry(egregore_id=egregore.id, item_id=pool_item.id))
    ...
    async def _start() -> uuid.UUID:
        async with async_session_factory() as session:
            camp = await start_camp(
                session, run_id, egregore_id, now=NOW, random_source=random.Random(1)
            )
