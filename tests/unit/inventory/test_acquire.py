...
def test_new_acquisition_starts_at_level_1(make_entry, make_item_definition):
    ...
    result = acquire_item(existing, new_item, hp=500, mp=100, base_max_hp=500)
...
def test_duplicate_acquisition_increases_level_by_five_without_new_slot(make_entry):
    ...
    result = acquire_item([existing_entry], duplicate_item, hp=500, mp=100, base_max_hp=500)
...
def test_new_acquisition_never_changes_max_mp(make_entry, make_item_definition):
    """Per obsidian/.../キャラクター/ステータス.md section 5: max MP is
    fixed at 100 for every combatant and is never affected by acquiring
    an item."""
    existing = []
    new_item = make_item_definition(item_id=1)
...
def test_duplicate_acquisition_and_level_up_never_changes_max_mp(make_entry):
    existing_entry = make_entry(slot=1, item_id=1, current_level=1)
    duplicate_item = existing_entry.definition
...
def test_unowned_item_rejected_when_inventory_full(make_entry, make_item_definition):
    ...
    result = acquire_item(full_inventory, new_item, hp=500, mp=100, base_max_hp=500)
...
def test_duplicate_acquisition_succeeds_even_when_inventory_full(make_entry):
    ...
    result = acquire_item(full_inventory, duplicate_item, hp=500, mp=100, base_max_hp=500)
...
def test_acquisition_clamps_current_hp_to_new_max_hp(make_entry, make_item_definition):
    existing = []
    shrinking_item = make_item_definition(
        item_id=1,
        base_stat_modifiers={"max_hp": -450},
        per_level_stat_modifiers={},
    )

    result = acquire_item(existing, shrinking_item, hp=500, mp=100, base_max_hp=500)

    assert result.max_hp == 50
    assert result.hp == 50


def test_acquisition_does_not_heal_current_hp_when_max_hp_increases(
    make_entry, make_item_definition
):
    existing = []
    growing_item = make_item_definition(
        item_id=1,
        base_stat_modifiers={"max_hp": 100},
        per_level_stat_modifiers={},
    )

    result = acquire_item(existing, growing_item, hp=200, mp=20, base_max_hp=500)

    assert result.max_hp == 600
    assert result.hp == 200


def test_acquisition_clamps_current_mp_to_100_when_it_exceeds_the_max(
    make_entry, make_item_definition
):
    existing = []
    new_item = make_item_definition(item_id=1)

    result = acquire_item(existing, new_item, hp=500, mp=150, base_max_hp=500)
