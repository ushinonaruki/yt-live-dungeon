...
def _make_run_enemy(hp: int, display_name: str = "スライム") -> tuple[RunEnemy, str]:
    enemy = RunEnemy()
    enemy.id = uuid.uuid4()
...
async def test_use_hinokinofuta_damages_enemy_survives():
    ...
    result = await service.use_hinokinofuta(run_id=run_id, adventurer=adv)
...
async def test_use_hinokinofuta_raises_when_no_alive_enemies():
    ...
    with pytest.raises(NoAliveEnemyError):
        await service.use_hinokinofuta(run_id=run_id, adventurer=adv)
